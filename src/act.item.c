...
extern sh_int donation_rooms[NUM_DONROOMS +1];
...
ACMD(do_put)
{
  ...
  if (*arg3 && is_number(arg1)) {
    ...
    theobj = arg2;
    thecont = arg3;
  } else if (*arg3 && !is_number(arg1)) {
    send_to_char("That can't be right!\r\n", ch);
    return;
  } else {
    theobj = arg1;
    thecont = arg2;
				...
				struct obj_data * cont, int mode)
{
  if (mode == FIND_OBJ_INV || can_take_obj(ch, obj)) {
    if (IS_CARRYING_N(ch) >= CAN_CARRY_N(ch))
      ...
      if (IS_OBJ_STAT(obj, ITEM_AUTOENGRAVE) && GET_OBJ_ENGRAVE(obj) == 0
	  && GET_OBJ_TYPE(obj) != ITEM_MONEY) {
	GET_OBJ_ENGRAVE(obj) = GET_IDNUM(ch);
	act("$n gets $p, which magically engraves itself to $m.", TRUE, ch, obj, 0, TO_ROOM);
	act("You get $p, which magically engraves itself to you.", FALSE, ch, obj, 0, TO_CHAR);
	return;
      } else {
		     ...
		     byte mode, const char *sname, sh_int RDR)
{
  ...
  if ((IS_OBJ_STAT(obj, ITEM_NODROP)) && (GET_LEVEL(ch) < LVL_OVERSEER)) {
...
ACMD(do_drop)
{
  ...
  case SCMD_DONATE:
    sname = "donate";
    mode = SCMD_DONATE;
    RDR = real_room(donation_rooms[number(0, NUM_DONROOMS)]);
		            ...
		            int amount)
{
  ...
  if ((GET_GOLD(ch) < amount) && (IS_NPC(ch) || (GET_LEVEL(ch) < LVL_GRGOD))) {
    send_to_char("You don't have that many coins!\r\n", ch);
    return;
  }
  send_to_char(OK, ch);
...
ACMD(do_eat)
{
  ...
  if (subcmd == SCMD_EAT) {
    ...
    act("You nibble a little bit of the $p", FALSE, ch, food, 0, TO_CHAR);
    act("$n tastes a little bit of $p.", TRUE, ch, food, 0, TO_ROOM);
  }

  amount = (subcmd == SCMD_EAT ? GET_OBJ_VAL(food, 0) : 1);

  if (GET_OBJ_TYPE(food) == ITEM_HERB) {
    
    if (subcmd == SCMD_EAT)
      mod = number(1, GET_LEVEL(ch)) * amount;
    else
      mod = 1;
    
    switch (GET_OBJ_VAL(food, 1)) {
    case 0:
      GET_HIT(ch) = MIN(GET_MAX_HIT(ch), (GET_HIT(ch) + mod));
      send_to_char("You feel your wounds healing.\r\n", ch);
      break;
    case 1:
      GET_MANA(ch) = MIN(GET_MAX_MANA(ch), (GET_MANA(ch) + mod));
      send_to_char("You feel your magical ability returning.\r\n", ch);
      break;
    case 2:
      GET_MOVE(ch) = MIN(GET_MAX_MOVE(ch), (GET_MOVE(ch) + mod));
      send_to_char("You feel your legs becoming less tired.\r\n", ch);
      break;
    case 3:
      GET_STAM(ch) = MIN(GET_MAX_STAM(ch), (GET_STAM(ch) + mod));
      send_to_char("You feel your stamina returning.\r\n", ch);
      break;
    default:
      mudlogf(TRUE, BRF, LVL_GOD, "SYSERR: Reached default case in Herb Code.");
      break;
    }
  ...
  if (GET_OBJ_VAL(food, 3) && (GET_LEVEL(ch) < LVL_OVERSEER)) {
...
void wear_message(struct char_data * ch, struct obj_data * obj, int where)
{
  const char *wear_messages[][2] = {
    ...
    {"$n puts $p on $s right ear.",
       "You put $p on your right ear."},

    {"$n puts $p on $s left ear.",
    ...
    {"$n puts $p in $s hair.",
       "You put $p in your hair."},

    {"$n slides $p around $s ankle.",
       "You slide $p around your ankle."},
    
    {"$n wears $p on $s chest.",
       "You wear $p on your chest."},
    
    {"$n wears $p around $s hips.",
       "You wear $p around your hips."}
...
void perform_wear(struct char_data * ch, struct obj_data * obj, int where)
{
  ...
  int wear_bitvectors[] = {
    ...
    ITEM_WEAR_EYES, ITEM_WEAR_HAIR, ITEM_WEAR_ANKLE };
  ...
  const char *already_wearing[] = {
    ...
    "You're already wearing something in your hair.",
    "You're already wearing something around your ankle."
  };
  ...
  /* for neck, finger, and wrist, ear, try pos 2 if pos 1 is already full */
  if ((where == WEAR_FINGER_R) || (where == WEAR_NECK_1)
      || (where == WEAR_WRIST_R) || (where == WEAR_EAR_1))
    if (GET_EQ(ch, where))
      where++;
  ...
  if(GET_OBJ_ENGRAVE(obj)) {
    if (GET_OBJ_ENGRAVE(obj) != GET_IDNUM(ch)) {
      act("$p does not belong to you.", FALSE, ch, obj, 0, TO_CHAR);
      act("$n realises that $p is not $s and decides not to use it.",
  ...
  if (GET_LEVEL(ch) < GET_OBJ_LEVEL(obj)) {
    act("You do not know how to use $p.", FALSE, ch, obj, 0, TO_CHAR);
    act("$n doesn't understand how to use $p.", FALSE, ch, obj, 0, TO_ROOM);
    return;
  }

  if (GET_LEVEL(ch) < LVL_OVERSEER) {
    if (is_zone_closed((room_vnum)GET_OBJ_VNUM(obj))) {
      send_to_char ("The gods have decided you can't use that yet!\r\n", ch);
      return;
    }

    if (((GET_OBJ_VNUM(obj) / 100) == ARENA_ZONE) && !ROOM_FLAGGED(IN_ROOM(ch), ROOM_ARENA)) {
      send_to_char ("You can't use that outside the arena!\r\n", ch);
      return;
    }
  }
...
int find_eq_pos(struct char_data * ch, struct obj_data * obj, char *arg)
{
  ...
  if (!arg || !*arg) {
    ...
    if (CAN_WEAR(obj, ITEM_WEAR_HAIR))        where = WEAR_HAIR;
