...
void affect_modify(struct char_data * ch, byte loc, sbyte mod, long bitv,
		   bool add, sh_int vnum, bool save) {
  int maxabil;
  ...
  /* ok we do some syslog stuff here */

  if (!save && (vnum != -1) && !IS_NPC(ch) ) {
	...
	mudlogf(CMP, MAX(LVL_IMMORT, GET_INVIS_LEV(ch)), TRUE,
		"EQUIP:: %s's %s has increased by %d with obj #%d.", GET_NAME(ch), 
		apply_types[(int)loc], mod, vnum);
      }
      break;
...
void affect_total(struct char_data * ch)
{
  ...
  for (i = 0; i < NUM_WEARS; i++) {
	...
	affect_modify(ch, GET_EQ(ch, i)->affected[j].location,
		      ...
		      GET_EQ(ch, i)->obj_flags.bitvector, FALSE, -1, TRUE);
  }


  for (af = ch->affected; af; af = af->next)
    affect_modify(ch, af->location, af->modifier, af->bitvector, FALSE, -1, TRUE);
  ...
  for (i = 0; i < NUM_WEARS; i++) {
	...
	affect_modify(ch, GET_EQ(ch, i)->affected[j].location,
		      GET_EQ(ch, i)->affected[j].modifier,
		      GET_EQ(ch, i)->obj_flags.bitvector, TRUE, -1, TRUE);
  }


  for (af = ch->affected; af; af = af->next)
    affect_modify(ch, af->location, af->modifier, af->bitvector, TRUE, -1, TRUE);
...
void affect_to_char(struct char_data * ch, struct affected_type * af)
{
  ...
  *affected_alloc = *af;
  ...
  affect_modify(ch, af->location, af->modifier, af->bitvector, TRUE, -1, 0);
  affect_total(ch);
}
...
void equip_char(struct char_data * ch, struct obj_data * obj, int pos, bool save)
{
  int j, mod;
  ...
  if ((IS_OBJ_STAT(obj, ITEM_ANTI_EVIL) && IS_EVIL(ch)) ||
    ...
    ((invalid_class(ch, obj)) && (GET_LEVEL(ch)<LVL_HERO))) {   
  ...
  if (GET_OBJ_TYPE(obj) == ITEM_ARMOR) {
    ...
    mod = apply_ac(ch, pos);
    if ((mod > 40) && !IS_NPC(ch) && (!save)) {
      mudlogf(CMP, MAX(LVL_IMMORT, GET_INVIS_LEV(ch)), TRUE,
	      "EQUIP:: %s's AC has increased by %d with obj #%d.", GET_NAME(ch), 
	      mod, GET_OBJ_VNUM(obj));
    }
    GET_AC(ch) -= mod;
  }
  ...
  for (j = 0; j < MAX_OBJ_AFFECT; j++)
    affect_modify(ch, obj->affected[j].location,
		  ...
		  obj->obj_flags.bitvector, TRUE, GET_OBJ_VNUM(obj), save);
...
struct obj_data *unequip_char(struct char_data * ch, int pos)
{
  ...
  for (j = 0; j < MAX_OBJ_AFFECT; j++)
    affect_modify(ch, obj->affected[j].location,
		  obj->affected[j].modifier,
		  obj->obj_flags.bitvector, FALSE, -1, TRUE);
...
struct obj_data *get_obj_vis(struct char_data * ch, char *name)
{
  ...
  for (i = object_list; i && (j <= number); i = i->next)
    if (isname(tmp, i->name))
	  ...
	  && (!i->worn_by || CAN_SEE(ch, i->worn_by)))
	if (++j == number)
	  return i;
