...
void House_boot(void)
{
  ...
  sh_int real_house, real_atrium, j, k;
  FILE *fl;
  ...
  while (!feof(fl) && num_of_houses < MAX_HOUSES) {
    ...
    /* check if guests still exist */
    for (j = 0; j < temp_house.num_of_guests; j++) {
      if(get_name_by_id(temp_house.guests[j]) == NULL) {
	/* delete them */
	for (k = j; k < temp_house.num_of_guests; k++)
	  temp_house.guests[k] = temp_house.guests[k + 1];
...
void hcontrol_destroy_house(struct char_data * ch, char *arg)
{
  ...
  if ((real_atrium = real_room(house_control[i].atrium)) < 0) {
    sprintf(buf, "SYSERR: House %d had invalid atrium %d!", atoi(arg),
  ...
  if ((real_house = real_room(house_control[i].vnum)) < 0) {
    sprintf(buf, "SYSERR: House %d had invalid vnum %d!", atoi(arg), house_control[i].vnum);
    log(buf);
  } else
...
ACMD(do_house)
{
  ...
  else if (GET_IDNUM(ch) != house_control[i].owner && GET_LEVEL(ch) <= LVL_GRGOD)
...
int House_can_enter(struct char_data * ch, room_vnum house)
{
  ...
  if (GET_LEVEL(ch) >= LVL_SUPREME || (i = find_house(house)) < 0)
    return 1;
