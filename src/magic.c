...
int mag_savingthrow(struct char_data * ch, int type)
{
  ...
  if (IS_NPC(ch)) /* NPCs use warrior tables according to some book */
    save = saving_throws[CLASS_HUMP][type][(int) GET_LEVEL(ch)];
  else
...
void affect_update(void)
{
  ...
  for (i = character_list; i; i = i->next)
	...
	af->duration = -1;	/* GRGODs only! unlimited */
      else {
...
{
  int is_hump = 0, is_stroke = 0;
  int dam = 0;

  if (victim == NULL || ch == NULL)
    return;

  is_hump = (GET_CLASS(ch) == CLASS_HUMP);
  is_stroke = (GET_CLASS(ch) == CLASS_STROKE);
  ...
  case SPELL_GODLY_ASSIST:
    dam = dice(15, 8) + GET_LEVEL(ch);
    break;
  ...
  case SPELL_FIRE_SPIT:
    dam = dice(MIN(12, GET_LEVEL(ch)), 8);
    break;
  case SPELL_GAS_SPIT:
    dam = dice(GET_LEVEL(ch), 12);
    break;
  case SPELL_FROST_SPIT:
    dam = dice(14, 10) + level;
    break;
...
#define MAXSHIELDS         4
#define SHIELD_SHIELD      0
#define SHIELD_ARMOR       1
#define SHIELD_BARKSKIN    2
#define SHIELD_STONESKIN   3

void remove_shield(int spellnum, int message, struct char_data *vict) {

  char *shld_msg[MAXSHIELDS] = {
    "your shield kicks in",
    "magical armour surrounds you",
    "your skin turns to bark",
    "your skin turns to stone"
  };

  if (affected_by_spell(vict, SPELL_STONESKIN) && spellnum != SPELL_STONESKIN) {
    sprintf (buf, "Your stoneskin fades and %s.\r\n", shld_msg[message]);
    send_to_char(buf, vict);
    affect_from_char(vict, SPELL_STONESKIN);
  } else if (affected_by_spell(vict, SPELL_BARKSKIN) && spellnum != SPELL_BARKSKIN) {
    sprintf (buf, "Your barkskin fades and %s.\r\n", shld_msg[message]);
    send_to_char(buf, vict);
    affect_from_char(vict, SPELL_BARKSKIN);
  } else if (affected_by_spell(vict, SPELL_ARMOR) && spellnum != SPELL_ARMOR) {
    sprintf (buf, "Your armor fades and %s.\r\n", shld_msg[message]);
    send_to_char(buf, vict);
    affect_from_char(vict, SPELL_ARMOR);
  } else if (affected_by_spell(vict, SPELL_SHIELD) && spellnum != SPELL_SHIELD) {
    sprintf (buf, "Your shield fades and %s.\r\n", shld_msg[message]);
    send_to_char(buf, vict);
    affect_from_char(vict, SPELL_SHIELD);
  } 
		      ...
		      int spellnum, int savetype)
{
  ...
  int is_hump = FALSE, is_stroke = FALSE;
  ...
  case SPELL_DOOM:
    ...
    af[1].bitvector = AFF_DOOM;

    af[2].location = APPLY_DAMROLL;
    af[2].modifier = -30;
    af[2].duration = 10 + level;
    af[2].bitvector = AFF_DOOM;

    accum_duration = TRUE;
    to_vict = "You are surrounded by a dark aura!";
    to_room = "A dark aura suddenly surrounds $n.";
    break;
  ...
  case SPELL_HASTE:
    af[0].duration = 5 + GET_LEVEL(ch);
    af[0].bitvector = AFF_HASTE;
    accum_duration = FALSE;
    to_vict = "Your leg muscles seem to grow.";
    to_room = "$n's leg muscles flex.";
    break;
  ...
  case SPELL_SHIELD:
    ...
    remove_shield(spellnum, SHIELD_SHIELD, victim);
  ...
  case SPELL_ARMOR:
    ...
    remove_shield(spellnum, SHIELD_ARMOR, victim);
    af[0].location = APPLY_AC;
    af[0].modifier = -20;
    af[0].duration = 24;
    accum_duration = TRUE;
  ...
  case SPELL_BARKSKIN:
    ...
    remove_shield(spellnum, SHIELD_BARKSKIN, victim);
    af[0].location = APPLY_AC;
    af[0].modifier = -30;
    af[0].duration = 4 + level;
    accum_duration = TRUE;   
    to_vict = "You feel your skin hardening.";    
    to_room = "$n skin seems to turn into bark.";
    break;

  case SPELL_STONESKIN:

    remove_shield(spellnum, SHIELD_STONESKIN, victim);
    af[0].location = APPLY_AC;
    af[0].modifier = -40;
    af[0].duration = 5 + level;
    accum_duration = TRUE;
    to_vict = "You feel your skin turn into granite.";
    to_room = "$n seems to turn to stone.";
    break;

  case SPELL_GLOW:
    af[0].duration = 1;
    af[0].bitvector = AFF_GLOW;
    to_vict = "You glow brightly.";
    to_room = "$n begins to glow brightly.";
    accum_duration = FALSE;
    break;
...
void mag_areas(int level, struct char_data * ch, int spellnum, int savetype)
{
  ...
  for (tch = world[ch->in_room].people; tch; tch = next_tch) {
    ...
    if (!IS_NPC(tch) && GET_LEVEL(tch) >= LVL_IMMORT)
      continue;
    if (!pk_allowed(ch, tch) && !clan_can_kill(ch, tch) && !IS_NPC(ch) && !IS_NPC(tch))
      continue;
...
static const char *mag_summon_msgs[] = {
  ...
  "$N rises up from the ground!\r\n",
  "$N floats down from the sky!\r\n",     /* 5 */
  "$N fades in before your eyes!\r\n",
  "$N appears from a thick black cloud!\r\n",
  "$N arrives suddenly from in a brilliant white light!\r\n",  /* 8 */
  "You animate a corpse!\r\n",
  "You clone yourself!\r\n",    /* 10 */
  "$n animates a corpse!\r\n",
  ...
  "As $n makes a strange magical gesture, you feel the ground shake.\r\n",
  "$n makes a strange magical gestures; you feel a chill down your spine.\r\n",  
  "$n magically divides!\r\n"  /* 18 */
};
...
static const char *mag_summon_fail_msgs[] = {
  ...
  "You decide its not such a good day for that.\r\n",
  "Oh dear, you don't seem to be doing to well!\r\n",
...
#define MOB_LESSER_ELEM		21  /* lesser */
#define MOB_NORM_ELEM		20   /* normal */
#define MOB_GRT_ELEM		22   /* greater */
#define MOB_DRAGON              24     /* dragon  */
#define MOB_ANC_DRAGON          25     /* ancient */
#define MOB_GATE_I		160
#define MOB_GATE_II		170
#define MOB_GATE_III		180
#define MOB_ELEMENTAL_BASE	23  /* crap elemental */
#define MOB_CLONE		10
#define MOB_ZOMBIE		11 /* animate dead */
#define MOB_AERIAL_SERVANT	19
		      ...
		      int spellnum, int savetype)
{
  ...
  int msg = 0, fmsg = 0, cmsg = 0;
  int mob_num = 0, r_num = 0;
  int handle_corpse = 0;
  ...
  if ((r_num = real_mobile(mob_num)) < 0) {
  ...
  act(mag_summon_msgs[msg], TRUE, ch, 0, mob, TO_ROOM);
  act(mag_summon_msgs[cmsg], TRUE, ch, 0, mob, TO_CHAR);
  
  add_follower(mob, ch);

  if (spellnum == SPELL_CLONE) {
    
    /* we give it his desc and name */

    strcpy(GET_NAME(mob), GET_NAME(ch));
    strcpy(mob->player.short_descr, GET_NAME(ch));

    /* ok we copy the stats over */
		     ...
		     int spellnum, int savetype)
{
  ...
  case SPELL_HEAL:
    ...
    break;
  case SPELL_GODLY_HEAL:
    hit = dice(4, 8) + (level << 10 );
    send_to_char("You feel the power of the gods rushing through your body\r\n", victim);
    break;
  }
		        ...
		        int spellnum, int type)
{
  ...
  case SPELL_GODLY_HEAL:
    spell = SPELL_DOOM;
    to_vict = "You no longer feel doomed!";
    to_room = "The colour returns to $n's skin.";
    break;
