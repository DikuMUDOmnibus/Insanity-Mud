   ...
   header (i.e. the text handed to the player) */
{
  ...
  if (!(mail_pointer = find_char_in_index(recipient))) {
    log("SYSERR: Mail system -- post office spec_proc error?  Error #7. (invalid char in index)");
    return 0;
  }
  if (!(position_pointer = mail_pointer->list_start)) {
    log("SYSERR: Mail system -- non-fatal error #8. (invalid position_pointer)");
    return 0;
  }
  ...
  if (header.block_type != HEADER_BLOCK) {
    log("SYSERR: Oh dear. (header.block_type == %ld, NOT %d)", header.block_type, HEADER_BLOCK);
    no_mail = 1;
    log("SYSERR: Mail system disabled!  -- Error #9. (invalid header_block)");
    return 0;
  }
  ...
  *(tmstr + strlen(tmstr) - 1) = '\0';
  ...
  sprintf(buf, " * * * * The Insane Mail System * * * *\r\n"
	  "Date: %s\r\n"
	  "  To: %s\r\n"
	  "From: %s\r\n\r\n", tmstr, to ? to : "Unknown",
	  from ? from : "Unknown");
			  ...
			  int cmd, char *arg)
{
  ...
  if (GET_LEVEL(ch) < LVL_IMMORT) {
    ...
    sprintf(buf, "$n tells you, 'Immortal Mail is Free!'\r\n"
	    "$n tells you, 'Write your message, (/s saves /h for help)'");
    act(buf, FALSE, mailman, 0, ch, TO_VICT);
  }

  SET_BIT(TMP_FLAGS(ch), TMP_MAILING | TMP_WRITING);
	...
	if (has_mail(GET_IDNUM(d->character))){
	  ...
	  if ( GET_POS(ch) > POS_SLEEPING && GET_POS(ch) != POS_FIGHTING)
		...
		obj->action_description = 
	      ...
	      act("An insane old man pops out of nowhere and gives you an envolope.",FALSE,ch,0 , 0, TO_CHAR);
	      act("An insane old man pops out of nowhere and gives $n an envelope.", FALSE, ch, 0, 0, TO_ROOM);
	    }
	}
  }
}
