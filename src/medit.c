...
void init_mobile(struct char_data *mob)
{
  ...
  GET_WEIGHT(mob) = 200;
  GET_HEIGHT(mob) = 198;
  GET_MOB_ATTACKS(mob) = 1;

  mob->real_abils.str = mob->real_abils.intel = mob->real_abils.wis = 13;
  mob->real_abils.dex = mob->real_abils.con = mob->real_abils.end = 13;
...
void medit_save_to_disk(int zone_num)
{
  ...
  if (!(mob_file = fopen(fname, "w"))) {
    mudlogf(BRF, LVL_OLC, TRUE, "SYSERR: OLC: Cannot open mob file!");
    return;
  }
  ...
  for (i = zone * 100; i <= top; i++) {
	...
	mudlogf(BRF, LVL_OLC, TRUE, "SYSERR: OLC: Cannot write mob file!");
        fclose(mob_file);
        return;
      }
      ...
      fprintf(mob_file, "%s~\n"
	      ...
	      GET_EXP(mob), GET_POS(mob), GET_DEFAULT_POS(mob), GET_SEX(mob), GET_MOB_ATTACKS(mob)
	      );
...
void medit_disp_mprog_types(struct descriptor_data *d)
{
  ...
  send_to_char("\r\n\r\nEnter mob prog type : ", d->character);
...
/*-------------------------------------------------------------------*/
  case MEDIT_MAIN_MENU:
    ...
    case 'N':
    case 'n':
      send_to_char("Enter Number of attacks: ", d->character);
      OLC_MODE(d) = MEDIT_MUL_HIT;
      return;
#if defined(OASIS_MPROG)
    case 'p':
    case 'P':
  ...
  case MEDIT_D_DESC:
    ...
    send_to_char("Oops... Somethings Gone wrong argh!!!\r\n", d->character);
    break;
  ...
  case MEDIT_MPROG:
    if ((i = atoi(arg)) == 0)
      ...
      send_to_char("\r\nWhich mob prog do you want to purge? ", d->character);
  ...
  case MEDIT_CHANGE_MPROG:
    if ((i = atoi(arg)) == 1)
      ...
      send_to_char ("\r\n\r\nEnter new arg list: ", d->character);
      OLC_MODE(d) = MEDIT_MPROG_ARGS;
    } else if (i == 3) {
      send_to_char("\r\n\r\nEnter new mob prog commands:\r\n", d->character);
      /*
 ...
 */
  ...
  case MEDIT_MPROG_TYPE:
    ...
    case 7:
      ...
      break;
    case 8:
      OLC_MPROG(d)->type = 128;
      break;
    case 9:
      OLC_MPROG(d)->type = 256;
      break;
    case 10:
      OLC_MPROG(d)->type = 512;
      break;
    case 11:
      OLC_MPROG(d)->type = 1024;
      break;
    default:
      OLC_MPROG(d)->type = -1;
      break;
  ...
  case MEDIT_HITROLL:
    GET_HITROLL(OLC_MOB(d)) = MAX(0, MIN(120, atoi(arg)));
    break;

  case MEDIT_DAMROLL:
    GET_DAMROLL(OLC_MOB(d)) = MAX(0, MIN(120, atoi(arg)));
    break;
  ...
  case MEDIT_EXP:
    GET_EXP(OLC_MOB(d)) = MAX(0, MIN(2000000, atoi(arg)));
    break;

  case MEDIT_GOLD:
    GET_GOLD(OLC_MOB(d)) = MAX(0, MIN(10000, atoi(arg)));
    break;
  ...
  case MEDIT_MUL_HIT:
    GET_MOB_ATTACKS(OLC_MOB(d)) = MAX(1, MIN(10, atoi(arg)));
    break;
