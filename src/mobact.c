...
void mobile_activity(void)
{
  ...
  for (ch = character_list; ch; ch = next_ch) {
	...
	if (IS_NPC(vict) || !CAN_SEE(ch, vict) || 
	    PRF_FLAGGED(vict, PRF_NOHASSLE) || ROOM_FLAGGED((ch->in_room), ROOM_PEACEFUL))
	  continue;
    ...
    /* clan mobs hit other clan member */
    if (MOB_FLAGGED(ch, MOB_CLAN)) {
      found = FALSE;
      for (vict = world[ch->in_room].people; vict && !found; vict = vict->next_in_room) {
	if (IS_NPC(vict) || !CAN_SEE(ch, vict) || PRF_FLAGGED(vict, PRF_NOHASSLE))
	  continue;

	clan = is_in_clan(vict);
	
	if (clan < 0 ) 
	  continue;
	
	if ( ((int)mob_index[ch->nr].virtual / 100) == clan_control[clan].castlezone ) 
	  continue;
