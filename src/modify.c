...
void parse_action(int command, char *string, struct descriptor_data *d)
{
  ...
  default:
    ...
    mudlogf(BRF, LVL_IMPL, TRUE, "SYSERR: invalid command passed to parse_action");
    return;
  }
}
...
void string_add(struct descriptor_data *d, char *str)
{
  ...
  if (terminator) {
	...
	*d->str = d->backstr;
    ...
    else if (!d->connected && (TMP_FLAGGED(d->character, TMP_MAILING))) {
	...
	if (!(fl = fopen((char *)d->storage, "w"))) {
	  mudlogf(CMP, LVL_IMPL, TRUE, "SYSERR: Can't write file '%s'.", d->storage);
	}
	else {
	  if (*d->str) {
	  ...
	  fclose(fl);
	  mudlogf(CMP, MAX(GET_INVIS_LEV(d->character), LVL_GOD), TRUE, 
		  "OLC: %s saves '%s'.", GET_NAME(d->character), d->storage);
	  SEND_TO_Q("Saved.\r\n", d);
	}
      }
    ...
    if (d->character && !IS_NPC(d->character))
      REMOVE_BIT(TMP_FLAGS(d->character), TMP_WRITING | TMP_OLC | TMP_MAILING);
    if (d->backstr) 
      free(d->backstr);
    d->backstr = NULL;
    d->str = NULL;
...
ACMD(do_skillset)
{
  ...
  mudlogf(BRF, -1, TRUE, "%s changed %s's %s to %d.", GET_NAME(ch),
