...
ACMD(do_olc)
{
  ...
  if (!*buf1) {                /* No argument given. */
    ...
    } else if (subcmd == SCMD_OLC_ZEDIT && GET_LEVEL(ch) >= LVL_CREATOR) {
  ...
  /*. Everyone but Creators and Impl can only edit zones they have been assigned .*/  
  /* and now those assigned with the world flag */
  if ((GET_LEVEL(ch) < LVL_CREATOR) && !PLR_FLAGGED(ch, PLR_WORLD) &&
      ((zone_table[OLC_ZNUM(d)].number != GET_OLC_ZONE1(ch)) &&
       (zone_table[OLC_ZNUM(d)].number != GET_OLC_ZONE2(ch)) &&
       (zone_table[OLC_ZNUM(d)].number != GET_OLC_ZONE3(ch)))) {
  ...
  if (save) {
    ...
    mudlogf(CMP, MAX(LVL_IMMORT, GET_INVIS_LEV(ch)), TRUE,
	    "OLC: %s saves %s for zone %d.", GET_NAME(ch), type,
  ...
  act("$n magically starts to build things.", 
      TRUE, d->character, 0, 0, TO_ROOM);
  SET_BIT(TMP_FLAGS(d->character), TMP_OLC);
}
/*------------------------------------------------------------*\
 Internal utlities 
\*------------------------------------------------------------*/
...
void cleanup_olc(struct descriptor_data *d, byte cleanup_type)
{
  if (d->olc) {
     ...
     */
    if (d->character) {
      REMOVE_BIT(TMP_FLAGS(d->character), TMP_OLC);
      STATE(d) = CON_PLAYING;
      act("$n stops building.", TRUE, d->character, 0, 0, TO_ROOM);
    }
    free(d->olc);
  }
}
