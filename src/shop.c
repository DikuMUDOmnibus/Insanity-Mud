...
const char *trade_letters[] = {
  ...
  "Evil",
  "Neutral",
  "Humpin",           /* Then the class based ones */
  "Bangin",
  "Strokin",
  "Effin",
  "Fumblin",
...
int is_ok_char(struct char_data * keeper, struct char_data * ch, int shop_nr)
{
  ...
  if (!(CAN_SEE(keeper, ch)) && GET_LEVEL(ch) < LVL_GOD) {
    do_say(keeper, (char *)MSG_NO_SEE_CHAR, cmd_say, 0); /* Evil cast ... */
    return (FALSE);
  }
  if (IS_GOD(ch))
    return (TRUE);
  ...
  if ((IS_HUMP(ch) && NOTRADE_HUMP(shop_nr)) ||
      (IS_STROKE(ch) && NOTRADE_STROKE(shop_nr)) ||
      (IS_EFF(ch) && NOTRADE_EFF(shop_nr)) ||
      (IS_DIG(ch) && NOTRADE_DIG(shop_nr)) ||
      (IS_FUM(ch) && NOTRADE_FUM(shop_nr)) ||
	  (IS_PURR(ch) && NOTRADE_PURR(shop_nr)) ||
      (IS_BANG(ch) && NOTRADE_BANG(shop_nr))) {
		       ...
		       struct char_data * keeper, int shop_nr)
{
  ...
  if ((buy_price(obj, shop_nr) > GET_GOLD(ch)) && !IS_GOD(ch)) {
    ...
    case 0:
      ...
      return;
    case 1:	/* Evil cast ... */
      do_echo(keeper, (char *)"smokes on his joint.", cmd_emote, SCMD_EMOTE);
      return;
    default:
      return;
    }
		            ...
		            struct char_data * keeper, int shop_nr, int msg)
{
  ...
  case OBJECT_DEAD:
    ...
    break;
  default:
    log("SYSERR: Illegal return value of %d from trade_with() (%s)", result, __FILE__);
