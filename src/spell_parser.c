...
const char *spells[] =
{
  ...
  /* 190 */
  "common", "humpin", "strokin", "bangin", "effin",	/* 195 */
  "fumblin", "diggin", "!UNUSED!", "!UNUSED!", "!UNUSED!", "!UNUSED!"	/* 200 */
	     ...
	     struct obj_data * ovict, int spellnum, int level, int casttype)
{
  ...
  if (casttype != CAST_BREATH || (casttype == CAST_BREATH && !IS_NPC(caster))) {
    if (ROOM_FLAGGED(caster->in_room, ROOM_NOMAGIC) && GET_LEVEL(caster) < LVL_GOD) {
  ...
  if (GET_LEVEL(caster) < LVL_GOD && IS_SET(ROOM_FLAGS(caster->in_room), ROOM_PEACEFUL) &&
...
void spell_level(int spell, int class, int level)
{
  ...
  if (class < 0 || class >= NUM_CLASSES) {
    log("SYSERR: assigning '%s' to illegal class %d/%d.",
	    skill_name(spell), class, NUM_CLASSES);
    bad = 1;
  }
...
void mag_assign_spells(void)
{
  ...
  spello(SPELL_SHIELD, 20, 15, 2,
         POS_STANDING, TAR_CHAR_ROOM, FALSE, MAG_AFFECTS);

  spello(SPELL_MINUTE_METEOR, 35, 15, 3, POS_FIGHTING,
	 TAR_IGNORE, TRUE, MAG_AREAS);

  spello(SPELL_CONE_OF_COLD, 36, 30, 3, POS_FIGHTING,
	 TAR_IGNORE, TRUE, MAG_AREAS);

  spello(SPELL_AREA_LIGHTNING, 100, 75, 3, POS_FIGHTING,
	 TAR_IGNORE, TRUE, MAG_AREAS);

  spello(SPELL_FIRE_SPIT, 100, 50, 4, POS_FIGHTING,
	 TAR_IGNORE, TRUE, MAG_AREAS);

  spello(SPELL_GAS_SPIT, 100, 50, 3, POS_FIGHTING,
	 TAR_IGNORE, TRUE, MAG_AREAS);

  spello(SPELL_FROST_SPIT, 100, 50, 4, POS_FIGHTING,
	 TAR_IGNORE, TRUE, MAG_AREAS);

  spello(SPELL_ACID_SPIT, 80, 80, 3, POS_FIGHTING,
	 TAR_IGNORE, TRUE, MAG_AREAS);

  spello(SPELL_LIGHTNING_SPIT, 100, 50, 5, POS_FIGHTING,
	 TAR_IGNORE, TRUE, MAG_AREAS);

  spello(SPELL_BLADEBARRIER, 55, 30, 3, POS_FIGHTING,
	 TAR_IGNORE, TRUE, MAG_AREAS);

  spello(SPELL_LEVITATE, 30, 15, 3, POS_STANDING, 
	 TAR_CHAR_ROOM, FALSE, MAG_AFFECTS);
  
  spello(SPELL_FLY, 50, 10, 2, POS_FIGHTING, 
	 TAR_CHAR_ROOM, FALSE, MAG_AFFECTS);

  spello(SPELL_WATERBREATH,  20, 10, 2, POS_STANDING, 
	 TAR_CHAR_ROOM, FALSE, MAG_AFFECTS);

  spello(SPELL_GROUP_FLY,  50, 10, 2, POS_FIGHTING, 
	 TAR_IGNORE, FALSE, MAG_GROUPS);

  spello(SPELL_GROUP_INVIS, 80, 50, 2, POS_STANDING, 
	 TAR_IGNORE, FALSE, MAG_GROUPS);

  spello(SPELL_GROUP_PROT_EVIL,  80, 50, 2, POS_FIGHTING, 
	 TAR_IGNORE, FALSE, MAG_GROUPS);

  spello(SPELL_ACID_ARROW,  30, 20, 2, POS_FIGHTING, 
	 TAR_CHAR_ROOM | TAR_FIGHT_VICT, TRUE, MAG_DAMAGE);

  spello(SPELL_FLAME_ARROW,  35, 20, 2, POS_FIGHTING, 
	 TAR_CHAR_ROOM | TAR_FIGHT_VICT, TRUE, MAG_DAMAGE);

  spello(SPELL_ARMOR, 30, 15, 3, POS_FIGHTING,
	TAR_CHAR_ROOM, FALSE, MAG_AFFECTS);

  spello(SPELL_STONESKIN, 60, 12, 3, POS_FIGHTING,
	 TAR_CHAR_ROOM, FALSE, MAG_AFFECTS);

  spello(SPELL_BARKSKIN, 45, 10,  3,
         POS_STANDING, TAR_CHAR_ROOM, FALSE, MAG_AFFECTS);

  spello(SPELL_GROUP_STONESKIN, 60, 30, 5, POS_FIGHTING,
	 TAR_CHAR_ROOM, FALSE, MAG_AFFECTS);
  ...
  spello(SPELL_DISINTEGRATE, 150, 100, 3, POS_FIGHTING, 
	 TAR_CHAR_ROOM | TAR_NOT_SELF | TAR_FIGHT_VICT, TRUE, MAG_MANUAL);
  
  spello(SPELL_FEAR, 150, 100, 3, POS_FIGHTING, 
  ...
  spello(SPELL_CONTROL_WEATHER, 75, 25, 5, POS_FIGHTING,
  ...
  spello(SPELL_GODLY_ASSIST, 160, 90, 5, POS_FIGHTING,
	TAR_CHAR_ROOM | TAR_FIGHT_VICT, TRUE, MAG_DAMAGE);

  spello(SPELL_GROUP_HEAL, 80, 60, 5, POS_FIGHTING,
	TAR_IGNORE, FALSE, MAG_GROUPS);

  spello(SPELL_GODLY_HEAL, 300, 200, 10, POS_FIGHTING,
	TAR_CHAR_ROOM, FALSE, MAG_POINTS | MAG_UNAFFECTS | MAG_AFFECTS);
  ...
  spello(SPELL_DOOM, 300, 160, 12, POS_STANDING,
	TAR_CHAR_ROOM, FALSE, MAG_AFFECTS);

  spello(SPELL_TELEPORT, 250, 180, 5, POS_STANDING,
  ...
  spello(SPELL_RELOCATE, 200, 150, 4, POS_STANDING,
	TAR_CHAR_WORLD | TAR_NOT_SELF, FALSE, MAG_MANUAL);
  
  spello(SPELL_PORTAL, 500, 350, 15, POS_STANDING,
	TAR_CHAR_WORLD | TAR_NOT_SELF, FALSE, MAG_MANUAL);
  ...
  spello(SPELL_ANIMATE_DEAD, 100, 80, 1,
         POS_STANDING, TAR_OBJ_ROOM, FALSE, MAG_SUMMONS);

    spello(SPELL_CONJ_ELEMENTAL, 80, 60, 2,
         POS_STANDING, TAR_IGNORE, FALSE, MAG_SUMMONS);

  spello(SPELL_MONSUM_I, 80, 60, 2,
         POS_STANDING, TAR_IGNORE, FALSE, MAG_SUMMONS);

  spello(SPELL_MONSUM_II, 80, 60, 2,
         POS_STANDING, TAR_IGNORE, FALSE, MAG_SUMMONS);

  spello(SPELL_MONSUM_III, 80, 60, 2,
         POS_STANDING, TAR_IGNORE, FALSE, MAG_SUMMONS);

  spello(SPELL_MONSUM_IV, 80, 60, 2,
         POS_STANDING, TAR_IGNORE, FALSE, MAG_SUMMONS);

  spello(SPELL_MONSUM_V, 80, 60, 2,
         POS_STANDING, TAR_IGNORE, FALSE, MAG_SUMMONS);

  spello(SPELL_HASTE, 50, 35, 1,
         POS_STANDING, TAR_CHAR_ROOM, FALSE, MAG_AFFECTS);
