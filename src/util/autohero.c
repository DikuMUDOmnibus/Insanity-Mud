...
void write_herolist(FILE * out, int minlev, int maxlev)
{
  char buf[100];
  struct level_rec *curr_level;
...
"|  \\   /\\|  M         oO     \\ \\`\\  \\ /  /'/ /     Oo        M  |/\\   /  |\r\n"
"|   \\ |  ` //          O    /~\\ \\,\\  |  /,/ /~\\    O         \\\\ '  | /   |\r\n"
"|  /'\\|    M      ______O  /__/ /__| I |__\\ \\__\\  O____       M    |/`\\  |\r\n"
" \\ |  `    \\\\     |      \\|  '''  ''' ```  ```  |/     |     //    '  | /\r\n"
"  \\|    |   M     |          Heros of Insanity         |     M   |    |/\r\n"
"   \\    \\\\_//     |                                    |     \\\\_//    /\r\n"
"    `    `~'      ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~      `~'    '\r\n"
"\r\n");
...
int main(int argc, char **argv)
{
  ...
  if (argc != 3 && argc != 4) {
    printf("Format: %s herolev herolistfile [pid to signal]\n",
	   argv[0]);
    exit(0);
  }
  herolevel = atoi(argv[1]);
  if (argc == 4)
    pid = atoi(argv[3]);
  initialize();
  read_file();
  sort_names();
  fl = fopen(argv[2], "w");
  write_herolist(fl, herolevel, LVL_PRINCE);
  fclose(fl);
  if (pid)
    kill(pid, SIGUSR1);
  return 0;
