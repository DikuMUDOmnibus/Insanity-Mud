...
{
  {LVL_IMMORT, "Newbie Helpers", "&w"},
  {LVL_OVERSEER, "Overseers", "&y"},
  {LVL_GUARDIAN, "Guardians", "&g"},
  {LVL_RULER, "Overlords", "&b"},
  {LVL_WIZARD, "Wizards", "&r"},
  {LVL_ARCHWIZ, "Arch-Wizards", "&m"},
  {LVL_GOD, "Lesser Gods", "&R"},
  {LVL_GRGOD, "Greater Gods", "&Y"},
  {LVL_SUPREME, "Supreme Gods", "&G"},
  {LVL_CREATOR, "Sub-Implementors", "&C"},
  {LVL_LOWIMPL, "Implementors", "&M"},
  {LVL_IMPL, "The Boss", "&M"},
  {0, "", ""}
};
...
void read_file(void)
{
  ...
  while (!feof(fl)) {
	...
	!(IS_SET(player.char_specials_saved.act, PLR_CRIMINAL)) &&
...
void write_wizlist(FILE * out, int minlev, int maxlev)
{
  ...
  fprintf(out,
"&w_________|-----------------------------------------------------------|________\r\n"
"\\       :| )\\   /(   These People have reached Immortality   )\\   /( |:      /\r\n"
"  \\     :|  )\\_/(    on Insanity and should be treated with   )\\_/(  |:    /\r\n"
"    >   :| (/^|^\\)  respect. They are also in charge so you  (/^|^\\) |:    > \r\n"
"  /     :|  \\*|*/           should do what they say.          \\*|*/  |:    \\\r\n"
"/_______:/---\\|/------------- Never annoy the Gods ------------\\|/---\\:______\\\r\n"
"              V                                                 V\r\n\r\n&n");
  ...
  for (curr_level = levels; curr_level; curr_level = curr_level->next) {
    ...
    for (j = 1; j <= i; j++)
      fputc(' ', out);
    fprintf(out, "%s%s%s\n", curr_level->params->colour_code, curr_level->params->level_name,
	    (curr_level->params->level >= LVL_GOD ? "&w" : "&W"));
    for (j = 1; j <= i; j++)
      fputc(' ', out);
