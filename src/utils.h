...
/* in log.c */
/*
 * The attribute specifies that log() is formatted just like printf() is.
 * 1,2 means the format string is the first parameter and start checking
 * the second parameter for the types in the format string.  Not certain
...
#define CRASH_FILE	0
#define ETEXT_FILE	1
#define ALIAS_FILE      2  /* Allows alias saving */
...
#define TMP_FLAGS(ch)         ((ch)->player_specials->tmp_flags)
#define TMP_FLAGGED(ch, f)    (IS_SET(TMP_FLAGS(ch), f))
...
#define GET_END(ch)     ((ch)->aff_abils.end)
...
#define GET_STAM(ch)	  ((ch)->points.stamina)
#define GET_MAX_STAM(ch)  ((ch)->points.max_stamina)
...
#define GET_QUEST(ch)           ((ch)->player_specials->saved.quest_points)
#define GET_GOHOME(ch)          ((ch)->player_specials->saved.gohome)
#define GET_MARRIED(ch)         ((ch)->player_specials->saved.married)
#define GET_OFFICE(ch)          ((ch)->player_specials->saved.office)
#define GET_PKDEATH(ch)          ((ch)->player_specials->saved.pkdeaths)
#define GET_PKKILLS(ch)          ((ch)->player_specials->saved.pkkills)
#define GET_MOBDEATH(ch)          ((ch)->player_specials->saved.mobdeaths)
...
#define GET_MINWIZ(ch)          ((ch)->player_specials->saved.min_wiz_lev)

/* Autoquest Macros */

#define GET_QUESTMOB(ch)     ((ch)->player_specials->questmob)
#define GET_QUESTOBJ(ch)     ((ch)->player_specials->questobj)
#define GET_QUESTGIVER(ch)   ((ch)->player_specials->questgiver)
#define GET_NEXTQUEST(ch)    ((ch)->player_specials->saved.nextquest)
#define GET_QUESTPOINTS(ch)  ((ch)->player_specials->saved.quest_points)
#define GET_COUNTDOWN(ch)    ((ch)->player_specials->countdown)
...
#define GET_MOB_ATTACKS(ch)     ((ch)->mob_specials.attacks)
...
/* room utils added by Nahaz 10/04/99 ************************************/
...
#define GET_OBJ_ATT(obj)	((obj)->obj_flags.attacks)
...
#define GET_OBJ_ENGRAVE(obj)	((obj)->obj_flags.engrave)
...
#define CAN_SEE(sub, obj) (SELF(sub, obj) || (sub->char_specials.saved.idnum == 1) || \
...
#define IS_HUMP(ch)  	        (!IS_NPC(ch) && \
				(GET_CLASS(ch) == CLASS_HUMP))
#define IS_STROKE(ch)		(!IS_NPC(ch) && \
				(GET_CLASS(ch) == CLASS_STROKE))
#define IS_BANG(ch)		(!IS_NPC(ch) && \
				(GET_CLASS(ch) == CLASS_BANG))
#define IS_EFF(ch)		(!IS_NPC(ch) && \
				(GET_CLASS(ch) == CLASS_EFF))
#define IS_FUM(ch)		(!IS_NPC(ch) && \
				(GET_CLASS(ch) == CLASS_FUM))
#define IS_DIG(ch)		(!IS_NPC(ch) && \
				(GET_CLASS(ch) == CLASS_DIG))
#define IS_PURR(ch)		(!IS_NPC(ch) && \
				(GET_CLASS(ch) == CLASS_PURR))
...
#define IS_NOSEX(ch)            (GET_SEX(ch) == SEX_NEUTRAL)
