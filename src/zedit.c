...
void zedit_create_index(int znum, char *type)
{
  ...
  if (!(oldfile = fopen(old_name, "r"))) {
    mudlogf(BRF, LVL_IMPL, TRUE, "SYSERR: OLC: Failed to open %s", buf);
    return;
...
void zedit_save_to_disk(int zone_num)
{
  ...
  if (!(zfile = fopen(fname, "w"))) {
    mudlogf(BRF, LVL_IMMORT, TRUE,
	    "SYSERR: OLC: zedit_save_to_disk:  Can't write zone %d.",
  ...
  for (subcmd = 0; ZCMD.command != 'S'; subcmd++) {
    ...
    case '*':
      ...
      continue;
    default:
      mudlogf(BRF, LVL_IMMORT, TRUE,"SYSERR: OLC: z_save_to_disk(): Unknown cmd '%c' - NOT saving", ZCMD.command );
      continue;
    }
...
void zedit_disp_arg1(struct descriptor_data *d)
{
  ...
  default:
    ...
    mudlogf(BRF, MAX(LVL_OLC, GET_INVIS_LEV(d->character)), TRUE, 
	    "SYSERR: OLC: zedit_disp_arg1(): Help!");
    return;
  }
}
...
void zedit_disp_arg2(struct descriptor_data *d)
{
  ...
  default:
    /*. We should never get here .*/
    cleanup_olc(d, CLEANUP_ALL);
    mudlogf(BRF, MAX(LVL_OLC, GET_INVIS_LEV(d->character)), TRUE, "SYSERR: OLC: zedit_disp_arg2(): Help!");
    return;
  }
...
void zedit_parse(struct descriptor_data * d, char *arg)
{
  ...
  case ZEDIT_MAIN_MENU:
    ...
    case 'q':
    case 'Q':
      if (OLC_ZONE(d)->age || OLC_ZONE(d)->number || OLC_ZONE(d)->top) {
    ...
    case 'T':
       ...
       * Edit top of zone.
       */
      if (GET_LEVEL(d->character) < LVL_LOWIMPL)
	zedit_disp_menu(d);
      else {
...
/*-------------------------------------------------------------------*/
  case ZEDIT_ARG1:
	...
	if (!has_zone_permission(atoi(arg), LVL_SUPREME, d->character)) {
		send_to_char("You can only load things from your zone, try again : ", d->character);
		return;
	}
  ...
  case ZEDIT_ARG2:
    ...
    case 'R':
		...
		if (!has_zone_permission(atoi(arg), LVL_SUPREME, d->character)) {
			send_to_char("You can only load into things from your zone, try again : ", d->character);
			break;
		}
...
/*-------------------------------------------------------------------*/
  case ZEDIT_MIN_LVL:
    ...
    if (!isdigit(*arg)) {
      ...
      if (pos > LVL_IMPL || pos < 0) {
        send_to_char("Value must be from 0 to LVL_IMPL. Minimum level? ", d->character);
      } else {
	OLC_ZONE(d)->lvl1 = pos;
  ...
  case ZEDIT_MAX_LVL:
    ...
    if (!isdigit(*arg)) {
      ...
      if (pos > LVL_IMPL || pos < 0) {
        send_to_char("Value must be from 0 to LVL_IMPL. Maximum level? ", d->character);
      } else {
	if (pos < OLC_ZONE(d)->lvl1) {
	  send_to_char("Value must be at least equal to the first. Maximum level? ", d->character);
	} else {
	  OLC_ZONE(d)->number = 1;
	  OLC_ZONE(d)->lvl2 = pos;
	  zedit_disp_menu(d);
